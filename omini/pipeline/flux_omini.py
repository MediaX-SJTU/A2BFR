...
def complex_imgaug(x, scale_size=(32, 32), input_size=(64, 64)):
    x = np.array(x)
    x = x[np.newaxis, :, :, :]
    aug_seq = iaa.Sequential([
        ...
        iaa.Resize(input_size, interpolation=ia.ALL),
    ])
    aug_img = aug_seq(images=x)
    return Image.fromarray(aug_img[0])

def simple_imgaug(x, input_size=(32, 32)):
    x = np.array(x)
    x = x[np.newaxis, :, :, :]
    aug_seq = iaa.Sequential([
        iaa.Resize(input_size, interpolation=ia.ALL),
...
def convert_to_condition(
    ...
    if condition_type == "depth":
        ...
        condition_image = simple_imgaug(raw_img.convert("RGB"))
